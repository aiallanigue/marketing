...
function wpinventory_get_display_settings( $type = 'listing' ) {
	...
	return array_filter( explode( ',', $display ) );
}
...
/**
 * Retreive the current item's media sources
 *
 * @param integer $limit - use 0 for no limit
 */
...
function wpinventory_media_tags( $media, $new_window = TRUE ) {
	echo wpinventory_get_media_tags( $media, $new_windows );
}
...
//			$data['item'] = $wpim_item->get( $data['inventory_id'] );

			do_action( 'wpim_reserve_sent', $data['inventory_id'], $data );
		}
 ...
 * Available filters:
 *
 * wpinventory_filter_sort_by_options        - the list of sort by fields that goes into the sort by dropdown
 * wpinventory_filter_sort_by_options_admin - same as above, but for admin page
 * wpinventory_filter_categories_options    - the list of categories that goes into the categories dropdown
 */