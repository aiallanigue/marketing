...
class WPIMLoop extends WPIMCore {
	...
	public function get_field( $fieldname ) {
		...
		if ( isset( $this->current_item->{$fieldname} ) ) {
			return $this->parse_field_for_display( $fieldname, $this->current_item->{$fieldname} );
		}

		// If they passed the shortname, grab it
		if ( isset( $this->current_item->{'inventory_' . $fieldname} ) ) {
			return $this->parse_field_for_display( 'inventory_' . $fieldname, $this->current_item->{'inventory_' . $fieldname} );
		}
	...
	public function get_permalink() {
		...
		if ( stripos( $permalink, '?' ) === FALSE && self::$seo_urls && $slug ) {
			...
			return $permalink .= $sep . 'inventory_id=' . $this->current_item->inventory_id;
		}
	}
